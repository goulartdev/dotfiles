export XDG_CONFIG_HOME := env('XDG_CONFIG_HOME', home_directory() / '.config')

scripts_dir := "./scripts"

sync-scripts:
  @chmod +x {{scripts_dir}}/sync-scripts
  @{{scripts_dir}}/sync-scripts

# create synlink to dotfiles under ./config to XDG_CONFIG_HOME
link-dotfiles:
  #!/usr/bin/env sh
  for f in ./config/*; do
    rm -rf $XDG_CONFIG_HOME/$(basename $f)
